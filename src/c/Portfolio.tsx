    ...
    <section className="min-h-screen flex items-center px-5">
      <div>
        <h1 className="text-5xl font-medium text-white text-balance uppercase">
          Portfolio<span className="animate-[ping_1s_infinite]">_</span>
        </h1>

        <div className="mt-5 flex border-collapse max-w-md flex-wrap">
          {loading ? (
            <p>Loading...</p>
          ) : error ? (
            <p>Error: {error}</p>
            ...
            pinnedRepos?.map((_, i) => (
              <div
                key={`${i}_${_?.name}`}
                className="border h-fit flex justify-center flex-col w-fit p-4 hover:bg-gray-50/10"
              >
                <h1 className="text-xl">{_?.name}</h1>
                <p className="text-sm opacity-80">{_?.description}</p>
                {/* <p className="flex items-center gap-2 py-2 font-mono">
                  <svg
                    className="size-4"
                    viewBox="0 0 24 24"
                    fill="none"
        ...
        {!loading && !error && (
          <Link
            target="_blank"
            href="https://github.com/mhlehri?tab=repositories"
            className="border px-4 py-2 inline-block mt-5 hover:bg-gray-50/10"
          >
            view more
          </Link>
        )}
